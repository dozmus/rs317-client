...
public final class Censor {
    ...
    public static boolean CENSORING = false;
    ...
    private static void readValues(Stream fragStream, Stream badEncStream,
            Stream domainEncStream, Stream tldListStream) {
        readBadEnc(badEncStream);
        readDomainEnc(domainEncStream);
        readFragmentsEnc(fragStream);
        readTldList(tldListStream);
    }

    private static void readTldList(Stream stream) {
        int i = stream.readUInt();
        aCharArrayArray624 = new char[i][];
        anIntArray625 = new int[i];

        for (int j = 0; j < i; j++) {
            anIntArray625[j] = stream.readUByte();
            char ac[] = new char[stream.readUByte()];

            for (int k = 0; k < ac.length; k++) {
                ac[k] = (char) stream.readUByte();
            }
    ...
    private static void readFragmentsEnc(Stream stream) {
        anIntArray620 = new int[stream.readUInt()];
    ...
    private static void method493(Stream stream, char ac[][], byte abyte0[][][]) {
        for (int j = 0; j < ac.length; j++) {
            char ac1[] = new char[stream.readUByte()];

            for (int k = 0; k < ac1.length; k++) {
                ac1[k] = (char) stream.readUByte();
            }
            ac[j] = ac1;
            byte abyte1[][] = new byte[stream.readUByte()][2];

            for (int l = 0; l < abyte1.length; l++) {
                abyte1[l][0] = (byte) stream.readUByte();
                abyte1[l][1] = (byte) stream.readUByte();
            }
    ...
    private static void method509(byte abyte0[][], char ac[], char ac1[]) {
        ...
        for (int k = 0; k <= ac.length - ac1.length; k += j) {
            ...
            while (l < ac.length && (!flag2 || !isDigitFlag)) {
                int k1;
                char c = ac[l];
                char c2 = '\0';
            ...
            if (i1 >= ac1.length && (!flag2 || !isDigitFlag)) {
                boolean flag4 = true;
    ...
    private static int hashCharArray(char ac[]) {
        if (ac.length > 6) {
            return 0;
        }
        int hash = 0;
